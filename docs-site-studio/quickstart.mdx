...
- A sibling checkout of [nodalmerge](https://docs.nodalmerge.com) — Studio embeds the
...
- Read [Concepts → How Studio works](/concepts/overview) for the mental model behind
  work units, projections, and branches
- Read [Concepts → Trust & autonomy](/concepts/trust-and-autonomy) to configure
  review policy, promotion branches, and experiments for less hands-on runs
- Browse the [Control Tower UI reference](/reference/ui-reference) for every panel
  and control
- If you're integrating from CI or an external system rather than the extension, see
  [Guides → Extending goals](/guides/extending-goals) and
