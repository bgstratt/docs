...
NodalMerge is a local-first distributed state engine built around:
...
- Embedding NodalMerge inside your own web app instead of running the standalone
  server? See `operators/server-setup#embedding-the-host-as-a-library` — the
  embeddable hosts hand you the request pipeline directly, so you configure CORS
  yourself rather than inheriting it from the package
