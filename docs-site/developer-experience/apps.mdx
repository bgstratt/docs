...
description: "Run the NodalMerge demo host, workflow demo, and replay lab playground from this repository."
...
- `http://127.0.0.1:4173` (`workflow-demo`)
...
This is the first dedicated playground for learning websocket/runtime behavior through direct observation.
...
This is the second flagship demo path and validates spatial shared-state interactions beyond document workflows.

## Browser smoke checklist

1. Install the app's built package output in a clean project and confirm it loads without console errors.
2. Connect to a room and confirm sync/presence behavior matches the dev-server experience.
3. Exercise the app's primary interaction (map pins, canvas nodes, text edits, etc.) and confirm state converges across two browser tabs.
4. Reload one tab mid-session and confirm it catches up to the shared state.
