...
Operations are the primitive NodalMerge is built from; projections are the
most powerful *consequence* of that primitive, not a second one — the same
history can become canonical state, a registered query view, a speculative
lane, or a materialization outside the state graph entirely, without
changing anything upstream. See
